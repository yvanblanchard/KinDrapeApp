...
def KinDrape_eff_NR(d, Grid, Org, Ang, OrgNode, PreShear, Plt, surface_type="hemisphere"):

    ## Initialize plot variables
    ax = None
    fig = None

    ## Create surface interpolator based on type
    x = np.linspace(-1, 1, 100)
    y = np.linspace(-1, 1, 100)
    X, Y = np.meshgrid(x, y)

    if surface_type.lower() == "hemisphere":
        Z = np.zeros_like(X)
    ...
    # Evaluate surface at origin point
    z_val = F(np.array([Org[0]]), np.array([Org[1]]))[0]
    Node[Idx[0][0],Idx[1][0]] = [Org[0], Org[1], z_val]
